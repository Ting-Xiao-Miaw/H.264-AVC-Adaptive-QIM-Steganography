      ...
      if (cbp & (1 << b8))  // test if the block contains any coefficients
      {
        for (j = block_y4; j < block_y4 + 8; j += BLOCK_SIZE)
        {
          for (i = block_x4; i < block_x4 + 8; i += BLOCK_SIZE)
          {
            ...
            int b4 = ((j - block_y4) >> 2) * 2 + ((i - block_x4) >> 2);  // calculate b4 idx
            short qdct_block[4][4] = { 0 };   // temp for qdct
            ...
            for (k = 0; k < numcoeff; ++k)
            {
              if (levarr[k] != 0)
              {
                ...
                qdct_block[j0][i0] = levarr[k]; // copy value to temp qdct
            ...
            if (currSlice->slice_type == I_SLICE && currMB->mb_type == I4MB) {
                extract_qdct_info_for_I4MB(currMB, b8, b4, qdct_block);
            }
          }
        }
      }
