...
void ParseCommand(InputParameters *p_Inp, int ac, char *av[])
{
  ...
  *p_Inp = cfgparams;
  ...
  while (CLcount < ac)
  {
    ...
    else if (0 == strncasecmp(av[CLcount], "-method", 5))  // embedding mode
    {
        if (CLcount + 1 >= ac)
            error("Missing argument for -method (use M, L, or P)", 300);

        char mode_char = av[CLcount + 1][0];
        switch (mode_char)
        {
        case 'M': embedding_method = 0; break;
        case 'L': embedding_method = 1; break;
        case 'P': embedding_method = 2; break;
        default:
            error("Invalid embedding method (choose M, L, or P)", 300);
        }
        CLcount += 2;
    }
