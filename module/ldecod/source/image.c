...
int decode_one_frame(DecoderParams *pDecoder)
{
  ...
  // init_qdct flag & store
  alloc_qdct_nonzero_flag(p_Vid);
  alloc_qdct_coeff_store(p_Vid);
  ...
  if (p_Vid->type == I_SLICE)
  {
      ...
      if (frame_id == 0) {
          init_mode_writer("mode.txt");
          init_mapping_writer("mapping.txt");
          if (embedding_method != -1)
            init_receiver_writer("receiver_message.txt");
      } // initial writer

      write_mode_info(p_Vid, frame_id);
      write_mapping_info(p_Vid, frame_id);
      if (embedding_method != -1)
        check_restore(p_Vid, frame_id);
      frame_id++;
  }
  free_qdct_nonzero_flag(p_Vid);    // free qdct flag
  free_qdct_coeff_store(p_Vid);     // free qdct store
