...
int encode_one_slice (VideoParameters *p_Vid, int SliceGroupId, int TotalCodedMBs)
{
  ...
  while (end_of_slice == FALSE) // loop over macroblocks
  {
    ...
    else
    {
      ...
      // zigzag scan list
      const int zigzag_scan[16][2] = {
          {0,0}, {0,1}, {1,0}, {2,0}, {1,1}, {0,2}, {0,3}, {1,2},
          {2,1}, {3,0}, {3,1}, {2,2}, {1,3}, {2,3}, {3,2}, {3,3}
      };


      if (embedding_method != -1 && currSlice->slice_type == I_SLICE && currMB->mb_type == I4MB) {
          for (int b8 = 0; b8 < 4; b8++) {
              for (int b4 = 0; b4 < 4; b4++) {

                  MappingEntry* cm = NULL;
                  if (next_mapping_available()) {
                      cm = get_current_mapping();
                  }
                  else
                      continue;

                  if (!(frame_id == cm->frame_id &&
                      currMB->mbAddrX == cm->mb_addr &&
                      b8 == cm->b8_idx &&
                      b4 == cm->b4_idx)) {
                      continue;
                  }
                  ...
                  // init coef matrix
                  int coef_matrix[4][4] = { 0 };
                  int zigzag_coef[16] = { 0 };
                  int index_in_zigzag = -1;
                  int level, run;
                  int scan_pos = 0;

                  // read all nonzero coef
                  while (scan_pos < 16 && currSlice->cofAC[b8][b4][0][scan_pos] != 0) {
                      level = currSlice->cofAC[b8][b4][0][scan_pos];
                      run = currSlice->cofAC[b8][b4][1][scan_pos];

                      index_in_zigzag += run + 1;

                      if (index_in_zigzag >= 16)
                          break;

                      int row = zigzag_scan[index_in_zigzag][0];
                      int col = zigzag_scan[index_in_zigzag][1];

                      coef_matrix[row][col] = level;

                      scan_pos++;
                  }

                  if (cm->category == 1) {
                      embed_category_1(coef_matrix);
                      update_mapping();
                  }
                  else if (cm->category == 2 || cm->category == 4) {
                      embed_category_2(coef_matrix);
                      update_mapping();
                  }
                  else if (cm->category == 3) {
                      embed_category_3(coef_matrix);
                      update_mapping();
                  }
                  else if (cm->category == 5) {
                      embed_category_5(coef_matrix);
                      update_mapping();
                  }


                  // turn qdct back to zig-zag order
                  for (int i = 0; i < 16; i++) {
                      int row = zigzag_scan[i][0];
                      int col = zigzag_scan[i][1];
                      zigzag_coef[i] = coef_matrix[row][col];
                  }

                  // re-generate run-level
                  int ac_idx = 0;
                  for (int i = 0; i < 16;) {
                      if (zigzag_coef[i] != 0) {
                          int run = 0;
                          int j = i - 1;
                          while (j >= 0 && zigzag_coef[j] == 0) {
                              run++;
                              j--;
                          }
                          currSlice->cofAC[b8][b4][0][ac_idx] = zigzag_coef[i];
                          currSlice->cofAC[b8][b4][1][ac_idx] = run;
                          ac_idx++;
                          i++;
                      }
                      else {
                  ...
                  // clean value
                  for (int i = ac_idx; i < 16; i++) {
                      currSlice->cofAC[b8][b4][0][i] = 0;
                      currSlice->cofAC[b8][b4][1][i] = 0;
                  }
              }
          }
      }
