...
int mode_decision_for_I4x4_blocks_JM_High (Macroblock *currMB, int  b8,  int  b4,  int  lambda,  distblk*  min_cost)
{
  ...
  int force_mode = -1;
  if (embedding_method != -1 && currSlice->slice_type == I_SLICE && next_mode_available()) {
      ModeEntry* cm = get_current_mode();
      if (frame_id == cm->frame_id &&
          ...
          force_mode = cm->pred_mode;  // get pred_mode
          update_mode();               // update next line
      }
  }
  ...
  for (ipmode = 0; ipmode < NO_INTRA_PMODE; ipmode++)
  {
    ...
    if( available_mode)
    {
      ...
      if (((rdcost < min_rdcost) || (rdcost == min_rdcost && ipmode == mostProbableMode))
          && ((force_mode == ipmode) || (force_mode == -1)))    // force mode
      {
        //--- set coefficients ---
...
int mode_decision_for_I16x16_MB_RDO (Macroblock* currMB, int lambda)
{
  ...
  for (k = 0;k < 4; k++)
  {
    ...
    if ((distortionY < min_rdcost - weighted_cost(lambda, 4))
        && ((force_mode == k) || (force_mode == -1)))   // force mode
    {
